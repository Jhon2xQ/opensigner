...
| Term | Definition |
|---|---|
| **End User** | The final user of the wallet (e.g., a player in a game). |
| **Developer / Project Owner** | The entity building the application (e.g., You). Controls the project secrets. |
| **Host / Operator** | The entity operating the infrastructure (e.g., Openfort or You in self-hosted). |
| **Auth Provider** | The system validating user identity (e.g., Openfort Auth, Google, Custom OIDC). |
...
:::warning[Custody Model]
**The open-source OpenSigner configuration is custodial by default** when using automatic recovery without 2FA. The backend can access both hot and cold shares to reconstruct user private keys.

For non-custodial deployments, you must use **password recovery**, **passkey recovery**, or enable **2FA for automatic recovery**.

→ See [Deployment Scenarios](/security/deployment-scenarios#understanding-the-custody-model) for complete details.
:::
...
- [Self-Hosting Guide](/security/self-hosting) - Migration guide and TEE recommendations
