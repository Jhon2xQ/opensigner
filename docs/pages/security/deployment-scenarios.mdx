...
**OpenSigner configuration is custodial by default** unless additional protections are introduced. This is by design—the open-source setup provides the infrastructure, but achieving non-custodial guarantees requires specific configuration choices.
:::
...
**In automatic recovery mode without 2FA: Yes.**

The backend has access to:
1. **Hot share** – accessible via a valid user JWT
2. **Cold share** – in automatic recovery mode, the cold share encryption is handled by the cold storage itself

With these two shares, the backend can coordinate to reconstruct the user's private key. This was validated through testing: using a valid user JWT, it is possible to access both shares and reconstruct/export a user's private key on the backend without any additional user-held secret.

**In password/passkey recovery or automatic recovery with 2FA: No.**

The backend cannot reconstruct the key because:
...
**Yes.** In the default OpenSigner open-source setup:

1. Both hot and cold storage are platform-controlled
2. Automatic recovery mode allows share decryption without user-held secrets
3. A valid JWT is sufficient to coordinate access to both shares
4. No additional user action is required beyond standard authentication

**To achieve non-custodial guarantees, you must:**
...
- Host hot and cold storage with **different, mutually-untrusting parties**
...
### Production vs Open-Source Configuration

| Aspect | Open-Source Default | Non-Custodial Production |
|--------|--------------------|-----------------------|
| Hot + Cold share access | Backend has access to both | Backend blocked from at least one |
| User-held secret required | No | Yes (password, passkey, or OTP) |
| Can reconstruct key server-side | Yes | No |
| Regulatory custody classification | Likely custodial | Non-custodial |
