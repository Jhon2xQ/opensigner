...
| Automatic (no 2FA) | ⚠️ Attack succeeds - credentials unlock all shares |
...
|----------------------|---------|
| Single provider hosts all + automatic recovery | ⚠️ Provider can reconstruct keys |
| Single provider + password/passkey recovery | ❌ User entropy protects cold share |
| Single provider + automatic + 2FA | ❌ OTP required for cold share access |
| Split hosting (different providers) | ❌ No single party has both shares |
