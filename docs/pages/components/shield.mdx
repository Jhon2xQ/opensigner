...
Shield is ready to be Dockerized. No environment variables or build args are needed for this step. A regular `docker build . -t xyz ...` does the trick.
...
Shield can also run locally (for example, `go cmd/main.go server`) **although this is strongly discouraged**.
...
Secure communications are essential here: Shield communicates unencrypted shares to the iFrame. Any compromise in any of the two endpoints or in the communication channel might give access to a share to unwanted audiences.
...
This type of share recovery also can be enhanced with 2FA, detailed description is provided below.
...
Having non-authenticated encryption is not a big issue here either: decrypting rubbish only makes further checks (such as private key to address derivation) fail.
...
## 2FA (Two-Factor Authentication) for automatic recovery
...
:::info
2FA is available only for shares with automatic recovery method.
:::
...
| 400 | `OTP_NOT_SUPPORTED` | Project doesn't support 2FA |
| 409 | `OTP_ALREADY_ENABLED` | Project already has 2FA enabled |
